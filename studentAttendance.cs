...
namespace To_do_List
{
    ...
    {
        public studentAttendance(string fullName, string imagePath)
        {
            InitializeComponent();
        ...
        private void RecordTimeIn(string studentID, string classID, DateTime timeIn)
        {
            ...
            try
            {
                ...
                string checkQuery = "SELECT COUNT(*) FROM studentattendance WHERE student_id = @studentID AND class_id = @classID AND date = @date";
                ...
                if (existingCount == 0) // No existing record found
                {
                    ...
                    string insertQuery = "INSERT INTO studentattendance (student_id, class_id, date, timeIN, inStatus) " +
        ...
        private void RecordTimeOut(string studentID, string classID, DateTime timeOut)
        {
            ...
            try
            {
                ...
                string updateQuery = "UPDATE studentattendance SET timeOut = @timeOut, outStatus = @outStatus WHERE student_id = @studentID AND class_id = @classID AND date = @date";
